    ...
    const fetchComprehensiveData = async () => {
      ...
      try {
        ...
        if (error.code === 'permission-denied') {
          setError('Access denied. Please make sure you are logged in with the correct account and have proper permissions.');
        } else if (error.code === 'unauthenticated') {
          setError('Please sign in to view your financial data.');
        } else if (error.code === 'unavailable') {
          setError('Firebase service is temporarily unavailable. Please try again later.');
        } else {
          setError('Failed to fetch financial data. Please check your internet connection and try again.');
        }
      } finally {
        setLoading(false);
      }
        ...
        {error && (
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
            ...
            <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
              <button onClick={() => setError('')} className="text-red-500 hover:text-red-900">
